...
@njit(fastmath=True,cache=True)
def fvg(s):
    ki,ko,kt=vg(s);W=len(s);nb=max(3,W//10);nt=min(10,W//3)
    gr=np.mean(kt[-nb:]);ga=max(1.0,np.mean(kt[:nb]));sl=0.0
    if nt>=3:
...
def ftr(s):
    ...
    md=np.mean(np.maximum(d,0.0));mu=np.mean(np.maximum(-d,0.0))
    dd=100.0 if W<15 or mu==0 else 100.0-100.0/(1.0+md/mu)
    pr=max(np.max(s)-np.min(s),1e-8);ma=np.mean(s[-20:]) if W>=20 else s[-1]
...
def bds_multi(p,v=40,horizons=np.array([1,3,5,10])):
    ...
    X=np.zeros((M,19),dtype=np.float64);D=np.zeros((M,nh),dtype=np.float64)
    R=np.zeros((M,nh),dtype=np.float64);P=np.zeros((M,nh),dtype=np.float64)
    for i in range(M):
        ...
        for hi in range(nh):
            h=int(horizons[hi])
            R[i,hi]=(p[i+v+h]-p[i+v])/p[i+v]
            D[i,hi]=1.0 if R[i,hi]>0 else 0.0
            P[i,hi]=p[i+v+h]
    return X,D,R,P
...
class TimesNetMetaWrapper:
    def __init__(self,n_features,seq_len=20,lr=1e-3,epochs=10,d_model=32,d_ff=32,e_layers=2,top_k=3,num_kernels=4,dropout=0.1):
        self.seq_len=seq_len
        self.lr=lr
        self.epochs=epochs
        self.n_features=n_features
        self.net=TimesNetMeta(enc_in=n_features,seq_len=seq_len,pred_len=1,d_model=d_model,d_ff=d_ff,e_layers=e_layers,top_k=top_k,num_kernels=num_kernels,dropout=dropout,c_out=1)
    ...
    def fit(self,X,y,sample_weight=None):
        ...
        Xs,ys=self._build_sequences(Xn,yn)
        if len(Xs)<2:
            return self
...
class VGSS:
    def __init__(self, tk="AAPL", v=40, horizons=[1,3,5,10], rt=0.7,
                 err_window=25,
                 err_thresh=15.0,
        ...
        print(f"✅ {len(self.D)} muestras | train={self.tr} | stride={self.stride}")
    ...
    def run_all(self):
        ...
        pbar_main = tqdm(transformaciones.items(), desc="🔄 Transformaciones", unit="transf", colour="green")
        for name,transformer in pbar_main:
            pbar_main.set_postfix_str(f"{name[:20]}")
            try:
        ...
        for name,acc,r2 in resultados:
            print(f"  {name:30s} │ Acc: {acc:.4f} │ R²: {r2:.4f}")
    ...
    def run_single_transform(self,t_name,transformer):
        ...
        bc=[
            CBC(verbose=0,iterations=80,random_seed=42,thread_count=-1),
            LC(n_estimators=80,verbose=-1,random_state=42,n_jobs=-1),
            XC(n_estimators=80,verbosity=0,eval_metric='logloss',random_state=42,n_jobs=-1)
        ]
        br_all=[[
            CBR(verbose=0,iterations=80,random_seed=42,thread_count=-1),
            LGR(n_estimators=80,verbose=-1,random_state=42,n_jobs=-1),
            XR(n_estimators=80,verbosity=0,random_state=42,n_jobs=-1)
        ] for _ in range(nh)]
        bc_mh=[[
            CBC(verbose=0,iterations=60,random_seed=42,thread_count=-1),
            LC(n_estimators=60,verbose=-1,random_state=42,n_jobs=-1),
            XC(n_estimators=60,verbosity=0,eval_metric='logloss',random_state=42,n_jobs=-1)
        ] for _ in range(nh)]
        ...
        # Fase 1
        with tqdm(total=3, desc=f"  ⏳ {t_name[:15]}", leave=False, colour="cyan") as pbar:
            pbar.set_description(f"  📊 Fase 1: Meta-features")
            ...
            Meta0=np.column_stack([Mc0,Mr0_all,Mh0_all])
            
            mc=LR(max_iter=1000,n_jobs=-1)
            mc.fit(Meta0,y_dir[t2:t])
            
            seq_len_safe=min(12,max(4,len(Meta0)//15))
            mr=TimesNetMetaWrapper(n_features=n_meta_feat,seq_len=seq_len_safe,lr=1e-3,epochs=10,d_model=32,d_ff=32,e_layers=2,top_k=2,num_kernels=3)
            mr.fit(Meta0,S_Z[t2:t,hi])
            pbar.update(1)
            
            # Fase 2
            pbar.set_description(f"  🔧 Fase 2: Re-train completo")
            ...
            # Fase 3 prep
            pbar.set_description(f"  🚀 Fase 3: Walk-forward")
            nt_total=len(y_dir)-t
            test_indices=list(range(0,nt_total,stride))
            nt=len(test_indices)
            
            bs0=t-t2
            Mcb=np.zeros((bs0+nt,n_meta_feat))
            ycb=np.zeros(bs0+nt)
            prb=np.zeros(bs0+nt)
        ...
        # Walk-forward con barra de progreso
        last_retrain=0
        n_retrains=0
        
        pbar_wf = tqdm(enumerate(test_indices), total=nt, desc=f"  🔮 Walk-forward", leave=False, colour="yellow")
        for step,off in pbar_wf:
            idx=t+off
            if idx>=len(Xs):
                break
            ...
            # Actualizar meta solo cada N pasos
            if step % self.meta_update_freq == 0 and step > 0:
                sw=np.exp(np.linspace(-2,0,bs))
            ...
            if need_retrain or (step-last_retrain>=self.retrain_interval and step>0):
                e=idx+1
                last_retrain=step
                n_retrains+=1
                ...
                recent_errors.clear()
                pbar_wf.set_postfix_str(f"🔄 retrain #{n_retrains}")
            
            # Actualizar barra cada 5 pasos
            if step % 5 == 0:
                acc_parcial = AC(y_dir[t:t+off+1:stride][:step+1], pd_a[:step+1]) if step > 0 else 0
                pbar_wf.set_postfix(acc=f"{acc_parcial:.3f}", retrains=n_retrains)
        ...
        tqdm.write(f"  ✅ {t_name}: Acc={a:.4f} │ R²={r2:.4f} │ Retrains={n_retrains}")
    ...
    def graficar(self,pe,pp_a,ye,pd_a,r2,a,t_name):
        ...
        a1.set_title(f'[{t_name}] {self.tk} │ R²={r2:.4f}',fontweight='bold')
        a1.legend();a1.set_ylabel('Precio ($)')
        a2.fill_between(range(n),-.1,1.1,where=ok,color='#2e7d32',alpha=.15)
        a2.fill_between(range(n),-.1,1.1,where=~ok,color='#c62828',alpha=.15)
        a2.step(range(n),ye,c='#1565c0',lw=1.5,where='mid',label='Real')
        a2.step(range(n),pd_a,c='#c62828',lw=1.1,where='mid',label='Pred',alpha=.7)
        a2.set_title(f'Dirección │ Acc={a:.4f}',fontweight='bold')
        a2.legend(ncol=2);a2.set_ylim(-.15,1.15)
        plt.tight_layout()
        plt.savefig(f'{self.tk}_{t_name}_adaptive.png',dpi=120,bbox_inches='tight')
        plt.close()

    def generar_html(self,resultados):
        html=f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Reporte {self.tk}</title>
<style>body{{font-family:Arial;background:#f5f5f5;padding:20px}}.container{{max-width:1000px;margin:auto;background:white;padding:20px;border-radius:8px}}
h1{{color:#1565c0;text-align:center}}table{{width:100%;border-collapse:collapse;margin:20px 0}}th,td{{padding:10px;text-align:center;border-bottom:1px solid #ddd}}
th{{background:#1565c0;color:white}}.best{{background:#c8e6c9}}img{{max-width:100%;margin:15px 0}}</style></head><body><div class="container">
<h1>📊 Reporte {self.tk}</h1><table><tr><th>Transformación</th><th>Accuracy</th><th>R²</th></tr>"""
        best_acc=max(r[1] for r in resultados)
        best_r2=max(r[2] for r in resultados)
        for name,acc,r2 in resultados:
            cls='best' if acc==best_acc or r2==best_r2 else ''
            html+=f"<tr class='{cls}'><td>{name}</td><td>{acc:.4f}</td><td>{r2:.4f}</td></tr>"
        html+="</table><h2>Visualizaciones</h2>"
        for name,_,_ in resultados:
            html+=f"<h3>{name}</h3><img src='{self.tk}_{name}_adaptive.png'>"
        html+="</div></body></html>"
        with open("reporte.html","w",encoding='utf-8') as f:
            f.write(html)
        print("\n✅ Reporte guardado: 'reporte.html'")
